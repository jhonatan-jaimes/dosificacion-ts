...
import { Mortero, Concreto, Recurso } from ".";
import { Dosificacion } from "./insumos";
...
function tableConcreto(dosificacion: string): Concreto {
...
export function table(dosificacion: string, tipo: string): Dosificacion{
  if (tipo == Recurso.CONCRETO){
    return tableConcreto(dosificacion);
  } else if (tipo == Recurso.MORTERO){
    return tableMortero(dosificacion);
  } else {
    throw new Error ("No se encontro el tipo en la tabla");
  }
}

function tableMortero(dosificacion: string): Mortero {
